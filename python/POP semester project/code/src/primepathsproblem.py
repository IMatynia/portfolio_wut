...
class PrimePathsProblem:
    def __init__(self):
        ...
        # self.final_opt_routes = 12
        # self.final_optimization_iters = 10000
    ...
    def solve(self):
        ...
        # two_opt_results = self.solve_2_opt_only()

        with open("[2-opt only]results_cache.bin", "rb") as file:
            two_opt_results = pickle.load(file)

        results = None
        # with open("results_cache.bin", "rb") as file:
        #     results = pickle.load(file)

        self.plot_results(results, two_opt_results)
    ...
    def plot_results(self, results, two_opt_results):
        ...
        raw_two_opt_data_min = []
        raw_two_opt_data_avg = []
        raw_two_opt_data_std = []

        for i in range(0, len(two_opt_results[0][2]), 100):
            ...
            two_opt_x.append(i)
            raw_two_opt_data_min.append(np.min([result[2][i] for result in two_opt_results]))
            raw_two_opt_data_avg.append(np.average([result[2][i] for result in two_opt_results]))
            raw_two_opt_data_std.append(np.std([result[2][i] for result in two_opt_results]))
        ...
        plt.plot(two_opt_x, raw_two_opt_data_min, label=f"Raw 2-opt min")
        plt.errorbar(two_opt_x, raw_two_opt_data_avg, raw_two_opt_data_std,
                     label="Averages over 12 runs")
